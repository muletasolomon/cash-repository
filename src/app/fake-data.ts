import {ZirZir} from './types';
import {Address} from './Addres';

export const fakeListings: ZirZir[] = [{
    id: '123',
    name: 'Old Boat',
    description: 'A very old boat. Bargain price',
    price: 700,
    views: 0,

}, {
    id: '345',
    name: 'Computer',
    description: 'From the 1990s, a classic!',
    price: 50,
    views: 0,
}, {
    id: '456',
    name: 'Basketball Hoop',
    description: 'Good condition, free delivery',
    price: 100,
    views: 0,
}];

export const fakeMyListings: ZirZir[] = [{
    id: '789',
    name: 'Bicycle',
    description: 'Bright blue, very fast',
    price: 75,
    views: 0,
}, {
    id: '234',
    name: 'Electric Guitar',
    description: 'Needs new strings, but otherwise in very good condition',
    price: 99,
    views: 0,
}, {
    id: '987',
    name: 'Playstation',
    description: 'Old playstation, still works though',
    price: 75,
    views: 0,
}];

export const contactInformations: Address[] = [{
  id: '123',
  email: 'meretsolomon@gmail.com',
  phone: 939657770,
  city: 'jima'
}, {
  id: '456',
  email: 'africa@gmail.com',
  phone: 939657770 ,
  city: 'addis'
}, {
  id: '345',
  email: 'ethiopia@gmail.com',
  phone: 939657770 ,
  city: 'dire'
}];
